...
@require_torch
class CpmBeeModelIntegrationTest(unittest.TestCase):
    @tooslow
    def test_inference_masked_lm(self):
        texts = "今天天气真好！"
        model_path = "openbmb/cpm-bee-10b"
        model = CpmBeeModel.from_pretrained(model_path)
        tokenizer = CpmBeeTokenizer.from_pretrained(model_path)
...
class CpmBeeForCausalLMlIntegrationTest(unittest.TestCase):
    ...
    def test_inference_casual(self):
        texts = "今天天气真好！"
        model_path = "openbmb/cpm-bee-10b"
        model = CpmBeeForCausalLM.from_pretrained(model_path)
        tokenizer = CpmBeeTokenizer.from_pretrained(model_path)
        inputs = tokenizer(texts, return_tensors="pt")
